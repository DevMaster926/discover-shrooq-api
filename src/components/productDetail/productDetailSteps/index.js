...
function ProductDetailSteps(props) {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (currentScrollY + clientHeight > document.documentElement.scrollHeight * 0.85) {
        showSticky(true);
      } else {
        showSticky(false);
      }
  ...
  return (
    <>
          ...
          {stepList?.map(({ id, icon, value, detail }, index) => (
            <div key={id} className={cx("item", "item-" + stepList.length)}>
              <div className={cx("btnWrapper", { active: id <= step, activeFull: id === step })}>
                <button
                  className={cx({ active: id <= step }) + (storeData.user.token  ? '' : 'disable-button')}
      ...
      <div className="render-content">
        ...
        {sub_name == "meals" && <FluentFood meals={meals} addMealPerson={addMealPerson} />}
        {sub_name == "transports" && (
          <Transport
            ...
            {isBooking ? (
              <div className={isSticky ? "book sticky row" : "book row"}>
                {realCoupon && (
                  <div
                    className="row"
                    style={{ width: "100%", fontSize: 14, backgroundColor: "#fff" }}
                  >
